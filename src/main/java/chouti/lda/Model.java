...
//				System.out.println("others line="+line);
			...
			logger.error("Error while reading other file:" + e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
	...
	protected boolean readTAssignFile(String tassignFile){
		try {
			...
			for (i = 0; i < M; i++){
				...
				for (j = 0; j < length; j++){
					...
					if (tknr2.countTokens() != 2){
						logger.error("Invalid word-topic assignment line\n");
						return false;
					}
			...
			logger.error("Error while loading model: " + e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
	...
	public boolean saveModelTAssign(String filename){
		...
		try{
			...
			logger.error("Error while saving model tassign: " + e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
	...
	public boolean saveModelTheta(String filename){
		try{
			...
			logger.error("Error while saving topic distribution file for this model: " + e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
	...
	public boolean saveModelPhi(String filename){
		try {
			...
			logger.error("Error while saving word-topic distribution:" + e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
	...
	public boolean saveModelOthers(String filename){
		try{
			...
			logger.error("Error while saving model others:" + e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
	...
	public boolean saveModelTwords(String filename){
		try{
			...
			logger.error("Error while saving model twords: " + e.getMessage());
			e.printStackTrace();
			return false;
		}
		return true;
	...
	public boolean initNewModel(LDAOption option){
		...
		data = LDADataset.readDataSet(dir + File.separator + dfile);//训练数据
		if (data == null){
			logger.error("Fail to read training data!\n");
			return false;
		}
	...
	public boolean initNewModel(LDAOption option, Model trnModel){
		...
		if (dataset == null){
			logger.error("Fail to read dataset!\n");
			return false;
		}
	...
	/**
	 * initJson parameter for continue estimating or for later inference
	 */
	public boolean initEstimatedModel(LDAOption option){
		...
		if (!loadModel()){
			logger.error("Fail to load word-topic assignment file of the model!\n");
			return false;
		}

		logger.info("Model loaded: " + "alpha: " + alpha + " | beta: " + beta + " | M: " + M + " | V: " + V);
